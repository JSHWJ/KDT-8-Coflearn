...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  for (let i = 0; i < Object.keys(myproj.data.userProject).length; i++) {
    ...
    a.setAttribute(
      "href",
      `/detailPage/${myproj.data.userProject[i].project_id}`
    );
  ...
  for (let i = 0; i < Object.keys(likeproj.data.cartProj).length; i++) {
    ...
    a.setAttribute(
      "href",
      `/detailPage/${likeproj.data.cartProj[i].project_id}`
    );
  ...
  for (let i = 0; i < Object.keys(myrecop.data.recop).length; i++) {
    ...
    re_people.innerText = `필요 인원 : 프론트엔드 [${myrecop.data.recop[i].front_num}/${myrecop.data.recop[i].frontG_num}] 백엔드 [${myrecop.data.recop[i].back_num}/${myrecop.data.recop[i].backG_num}]`;
    re_now.innerText = `모집 상태 : ${nowarr_my[i]}`;

    re_cent.style.color = "red";
    const percent =
      (myrecop.data.recop[i].crr_num / myrecop.data.recop[i].goal) * 100;
